...
class GreedySolver:
    ...
    def _create_symmetric_pair_relations(self, pair_relations):
    ...
    def _create_symmetric_pair_probabilities(self, pair_probabilities):
    ...
    def solve(self) -> dict:
        ...
        while len(self.parts_to_place) > 0:
            ...
            for i, slot in enumerate(self.next_slot_candidates):
                candidate, prob = self._find_best_candidate_for_slot(slot)
    ...
    def _find_best_candidate_for_slot(self, slot):
        part_relations = self._get_neighbours_and_relations(slot)
    ...
    def _get_neighbours_and_relations(self, slot):
    ...
    @staticmethod
    def _is_above(slot, other):
        return True if slot[0] + 1 == other[0] and slot[1] == other[1] else False

    @staticmethod
    def _is_below(slot, other):
        return True if slot[0] - 1 == other[0] and slot[1] == other[1] else False

    @staticmethod
    def _is_left(slot, other):
        return True if slot[0] == other[0] and slot[1] + 1 == other[1] else False

    @staticmethod
    def _is_right(slot, other):
        return True if slot[0] == other[0] and slot[1] - 1 == other[1] else False

    def _get_slot_relation(self, slot, other):
        if self._is_above(slot, other):
            return 0
        elif self._is_left(slot, other):
            return 1
        elif self._is_below(slot, other):
            return 2
        elif self._is_right(slot, other):
            return 3
        else:
            return 4
