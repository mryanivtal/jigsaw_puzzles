...
def get_callbacks(params: dict, outputs_path: str):
    ...
    if params['task'] == 'jigsaw_bin_class':
        callbacks = [
            JigsawLossAccuracyCsvCallback(train_csv_log_path, train=True, validation=True, test=False),
            JigsawLossAccuracyCsvCallback(test_csv_log_path, train=False, validation=False, test=True),
            JigsawPerSampleCsvCallback(per_sample_test_csv_log_path, train=False, validation=False, test=True),
