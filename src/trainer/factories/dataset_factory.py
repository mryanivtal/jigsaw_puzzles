...
def get_datasets(dataset_params, train_data_path, test_data_path) -> tuple:
    if dataset_params['dataset_type'] == 'plain':
        train_val_dataset, test_dataset = _get_plain_image_datasets(dataset_params, train_data_path, test_data_path)

    elif dataset_params['dataset_type'] == 'jigsaw':
        train_val_dataset, test_dataset = _get_jigsaw_datasets(dataset_params, train_data_path, test_data_path)

    elif dataset_params['dataset_type'] == 'patch_adjacence':
        train_val_dataset, test_dataset = _get_patch_adj_datasets(dataset_params, train_data_path, test_data_path)
...
def _get_jigsaw_datasets(dataset_params, train_data_path, test_data_path) -> tuple:
    # --- Transforms
    ...
    dataset_params['transforms']['resize_x'] = round(
        dataset_params['transforms']['resize_x'] / parts_x) * parts_x
    dataset_params['transforms']['resize_y'] = round(
        dataset_params['transforms']['resize_y'] / parts_y) * parts_y
    ...
    train_val_dataset = DogsVsCatsJigsawDataset(train_data_path, scrambler_params, target=dataset_target,
                                                transform=train_transform,
                                                transform_for_display=train_transform_for_display, shuffle=True)
    test_dataset = DogsVsCatsJigsawDataset(test_data_path, scrambler_params, target=dataset_target,
                                           transform=predict_transform,
                                           transform_for_display=predict_transform_for_display, shuffle=False)

    return train_val_dataset, test_dataset

def _get_patch_adj_datasets(dataset_params, train_data_path, test_data_path) -> tuple:
    # --- params
    transform_params = dataset_params['transforms']
...
def _get_plain_image_datasets(dataset_params, train_data_path, test_data_path) -> tuple:
    # --- Transforms
    transform_params = dataset_params['transforms']
